...
func (n *Notes) Update(msg tea.Msg) (common.Model, tea.Cmd) {
	...
	case FileContentMsg:
		...
		// Use glamour for markdown files, otherwise show line numbers
		if common.IsFileMarkdown(n.currentContent.content, n.currentContent.ext) {
			n.code.UseGlamour = true
			n.code.ShowLineNumber = false
		} else {
			n.code.UseGlamour = false
			n.code.ShowLineNumber = true
		}
